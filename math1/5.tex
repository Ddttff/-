...
\[1,\; x^2-1;\quad 2,\; \frac{x^2}{2}-\frac{1}{2},\; \frac{x}{2}+\frac{1}{2},\; \frac{x}{2}-\frac{1}{2};\quad \frac{1}{5},\; 5x^2-5,\; 5x+5,\; 5x-5; \cdots \]
一般地可以有因式：
\[a,\quad \frac{1}{a}x^2-\frac{1}{a},\quad \frac{1}{a}x+\frac{1}{a},\quad \frac{1}{a}x-\frac{1}{a},\cdots\]
...
\begin{enumerate}
    \item 填空：
\[36x^2y^2z=-4x^2y(\quad\quad),\quad x(y-z)=(\quad\quad)(x-y)  \]
\[2x(x-y)^2=(\quad\quad)(y-x)^2,\quad 7y(y-x)^3=(\quad\quad)(x-y)^3\]
\[-x-x^2-x^3=-x(\quad\quad),\quad 6a^2b+12ab-18ab^2=6ab(\quad\quad)\]
...
\begin{enumerate}
    \item 分解因式（口答）：
    \[a^2+10a+25,\quad 9-6b+b^2,\quad 4y^2-4y+1,\quad 1+2x+x^2,\quad 1+\frac{a^2}{9}-\frac{2}{3}a,\quad x^2+\frac{1}{x^2}+2\] 
    \item 分解因式：
     \[64m^2+48mn+9n^2,\quad 2ab-a^2-b^2,\quad -b+2bx-bx^2,\quad x^4-2x^2y^2+y^4\] 
     \[(m+n)^2-(2m+2n)(2m+n)+(2m+n)^2,\quad4a^2-4n^2+b^2-m^2+4(ab+mn) \]
\end{enumerate}
\end{ex}
...
\begin{enumerate}
    \item 填空：
    \[a^3+(\quad \quad)+(\quad\quad )+27b^3=(a+3b)^3\]
    \[27x^3+(\quad\quad)+(\quad\quad )-64y^3=(3x-4y)^3\]
    \[1+(\quad\quad)+(\quad\quad)+8a^6b^6=(1+2a^2b^2)^3\]
...
2.\quad$a^6-125b^3=(a^2-5b)(a^4+5a^2b+25b^2)$\\
3.\[\begin{split}
...
\begin{solution}
 ...
 \end{split}\]  
 3.\quad$8x^2+8x+2=8\left(x^2+x+\frac{1}{4}\right) =8\left(x+\frac{1}{2}\right)^2$
\end{solution}
...
\begin{enumerate}
    \begin{multicols}{2}
         \item $-3a^3b^2c=abc(\quad\quad )$
    \item $mx^2+nx-p=m(\quad \quad)$
    \end{multicols}
    \item $-12a^2b-3a^2b^2x+27ab^2=3ab(\quad\quad )$
    \item $3x^2y^3z^4-6x^3y^4z^2+18x^4y^2z^3=-3x^2y^2z^2(\quad\quad )$
\end{enumerate}
    \item 写出下列各题中的公因式：
\begin{enumerate}
    \begin{multicols}{2}
           \item $5x^2y, 15xy^2, 45xyz$
    \item $24a^2(1-x)^2(x+y),42ab(x-1)(x+y)^2$
    \item $-2(a+b)^2+4(a+b),(a+b)^2-3(a+b)^3$
    \item $(2x-4y)(2a-b),(x-y)(4a-2b)$
    \end{multicols}
\end{enumerate}
    \item 用提取公因式法分解因式：
    \begin{enumerate}
...
\begin{center}
    \begin{tikzpicture}
    \node at (0,0.5) [right] {$1\;\quad +3\;\quad  -1\;\;\quad\quad\quad  -6$};
    \node at (0,0) [right] {$\quad  a\; \quad \quad a^3+3a\;\quad\quad  a^3+3a^2-a$};
...
\begin{enumerate}
    \begin{multicols}{2}
    \item $x-2$是$f(x)=\poly{1,-5,12,-12}$的因式；\quad
...
\begin{proof}
假如一个$n$次多项式$f(x)$有$n+1$个不同的根$a_1,a_2,\cdots,a_n,a_{n+1}$，那么由推论2就知：
...
\begin{blk}{推论4}
    如果两个$n$次多项式$f(x),\; g(x)$在$x$取$a_1,a_2,a_3,\cdots,a_n,a_{n+1}$这$n+1$个不同的值时，
    它们所对应的值都相等，即
\[f (a_1) =g (a_1) ,\; \; f (a_2) =g (a_2) ,\;  \cdots,\;  f (a_{n+1}) =g (a_{n+1}) \]
那么，这两个多项式实是同一个多项式．即
\[f (x) =g (x) \]
\end{blk}
...
又由已知，$f (a_1) =g (a_1) ,\; \; f (a_2) =g (a_2) ,\;  \cdots,\;  f (a_{n+1}) =g (a_{n+1})$，可以得出：$f (a_1) -g (a_1)=0 ,\; \; f (a_2) -g (a_2)=0 ,\;  \cdots,\;  f (a_{n+1}) -g (a_{n+1})=0$．这就是说，$a_1,a_2,\cdots,a_{n+1}$都是多项式$f(x)-g(x)$的根．这就是说：$f(x)-g(x)$的次数不超过$n$次，却有$n+1$个不同的根．这显然与推论3的结论是矛盾的．所以是不可能的．
...
\begin{solution}
$\because\quad f(1)\ne 0,\quad \therefore\quad x-1$不是$f(x)$的因式．

$\because\quad f(-1)= 0,\quad \therefore\quad x+1$是$f(x)$的因式．

同样，$\because\quad f(2)= 0,\quad \therefore\quad x-2$是$f(x)$的因式．

$\because\quad f(3)= 0,\quad \therefore\quad x-3$是$f(x)$的因式．
...
应当指出，像例5.38中，$f(x)$是一个三次式，在找到一个因式$x+1$后，可不必再去试了，只要用综合除法就可将$f(x)$分解了．
\begin{center}
...
原方程的有理根是：
\[x_1=x_2=-2,\quad x_3=3,\quad x_4=\frac{1}{2}\]
\end{solution}
...
比较上边等式两边同类项的系数，得
\[b=-a(x_1+x_2),\quad c=ax_1\cdot x_2 \]
\[\therefore\quad x_1+x_2=-\frac{b}{a},\quad x_1\cdot x_2=\frac{c}{a}\]
...
\[\begin{split}
    ...
    &\quad +a(x_1x_2+x_1x_3+x_2x_3)x-ax_1x_2x_3
\end{split}\]
...
有$n$个根$x_1,x_2,\cdots,x_n$．那么，这$n$个根与系数之间有以下关系：
\begin{itemize}
...
\begin{solution}
    ...
    则由韦达定理知：$x_1+x_2=\frac{1}{2},\quad x_1x_2=-\frac{15}{2}$．
...
\item 当$a$为何值时：
\begin{enumerate}
    \item $f(x)=2x^5-3x^3+11x^2-x+a$除以$x+2$能余3？
...
\[\because\quad (3,5)=1,\quad \therefore\quad \text{3与5是互质的}\]
...
\begin{enumerate}
    \item 求下列各组数的最大公约数：
    \[135,\; 105;\quad 330,\; 825,\; 1485\]
    \item 求下列各组多项式的最高公因式：
\begin{enumerate}
    \item $12x^3y^2z^3,\quad 18x^2y^4z^2,\quad 30x^4yz^3$
    \item $(a+b)^2(a-b),\quad (a+b)(a-b)^2,\quad a^3b-ab^3$
    \item $x^2-1,\quad x^2+2x+1,\quad x^3+1$
    \item $y^2+5y+6,\quad y^2+y-2,\quad y^2-14y-32$
    \item $x^2+6x+9,\quad (x^3+3x^2+3x+9)(x+3)$
\end{enumerate}
\end{enumerate}    
\end{ex}
...
$$a=q_1\cdot b+r_1,\quad \text{其中: }0\le r_1<b$$
...
如果还嫌$b,r_1$数目太大，当然还可以重复以上的做法，用$r_1$，除$b$，得商$q_2$，余$r_2$，因而就又有：
...
% \foreach \y/\ytext in {0/x^4+x^3+0+0-2,-1/x^4+\,0+\,0-x\quad,-2/x^3+0+x-2,-3/x^3+0+0-1,-4/x-1}    
% {
%     \node at (0,\y)[left] {$\ytext$};
% }
...
\begin{center}
\begin{tikzpicture}[yscale=.6]
\foreach \y/\ytext in {6/1+0+0\quad -1  ,5/1-1\quad\quad \;\;  ,4/ 1+0\quad\; -1  , 3/ 1-1\quad \quad , 2/1-1  , 1/1-1 ,0/ 0}
{
    \node at (-.2,\y)[left]{$\ytext$};
}

\foreach \y/\ytext in {6/1+1+0+0-2  ,5/1+0+0-1\quad ,4/ 1+0+1-2  , 3/ 1+0+0-1  , 2/\boxed{1-1}}
{
...
\begin{center}
\begin{tikzpicture}[yscale=.6]
\foreach \y/\ytext in {6/1+0+3+1+2+1   ,5/1-1+1-1\quad\quad\;\;   ,4/1+2+2+2+1  ,3/ 1-1+1-1\quad   , 2/ 3+1+3+1 , 1/3-3+3-3, 0/4+0+4 }
{
    \node at (-.2,\y)[left]{$\ytext$};
}
...
\begin{center}
\begin{tikzpicture}[yscale=.6]
\foreach \y/\ytext in {6/1+0+3+1+2+1   ,5/1-1+1-1\quad\quad\;\;   ,4/1+2+2+2+1  ,3/ 1-1+1-1\quad   , 2/ 3+1+3+1 , 1/3-3+3-3, 0/4+0+4, -1/\boxed{1+0+1} }
{
    \node at (-.2,\y)[left]{$\ytext$};
}

\foreach \y/\ytext in {6/1+0+3+0+3+0+1  ,5/1+0+3+1+2+1\quad  ,4/ -1+1-1+1  , 3/ 1-1+1-1 , 2/1+0+1\quad , 1/-1+0-1, 0/-1+0-1, -1/0}
{
    \node at (4.5,\y)[left]{$\ytext$};
}
\foreach \x in {-4,0,4.5}
...
\begin{center}
%\begin{tikzpicture}[yscale=.6]
%\foreach \y/\ytext in {6/1+3+3+1   ,5/2+6+6+2  ,4/2+3+1\quad   ,3/ 3+5+2  , 1.5/ 3+\frac{9}{2}+\frac{3}{2} , 0/\frac{1}{2}+\frac{1}{2}, -1.5/\boxed{1+1} }
%{
%    \node at (-.2,\y)[left]{$\ytext$};
%}
%\foreach \y/\ytext in {6/1+0+0+0+0+1  ,5/1+3+3+1\quad\quad\;\;   ,4/ -3-3-1+0+1  , 3/ -3-9-9-3\quad\;\;  , 2/6\;+8\;\;+3\;+1 , 1/6+18+18+6, 0/-10-15-5, -1/2\;+3\;+1, -2/2+2\quad\;\;\;\; , -3/1+1,  -4/1+1, -5/0 }
%{
...
%\end{tikzpicture}
\includegraphics[width=15cm]{image/suanshi.png}
\end{center}
...
如果要求两个以上多项式$f_1(x),f_2(x),f_3(x),\cdots$的最高公因式时，只要先求$(f_1(x),f_2(x))=d_1(x)$, 再求$(d_1(x),f_3(x))=d_2(x)$, 再求$(d_2(x), f_4 (x))=d_3(x),\cdots$, 这样依次求下去，即可求得：
...
\begin{center}
\begin{tikzpicture}[yscale=.6]
\foreach \y/\ytext in {6/1+1-1+1-2   ,5/1+0-3+2\quad  ,4/1+2-1-2   ,3/ 1+0-3+2  , 2/2+2-4 ,1/\boxed{1+1-2} }
{
    \node at (.3,\y)[left]{$\ytext$};
}

\foreach \y/\ytext in {6/2+5-2-7+2  ,5/2+2-2+2-4  ,4/ 3+0-9+6 , 3/ 1+0-3+2  , 2/1+1-2\quad  , 1/-1-1+2, 0/-1-1+2, -1/0}
{
    \node at (3.8,\y)[left]{$\ytext$};
}
...
\foreach \y/\ytext in {6/3-1-1+0-1  ,5/3+0-3\quad\;\; \quad ,4/ -1+2+0-1 , 3/ -1+0+1\quad   , 2/2-1-1  , 1/2+0-2, 0/-1+1, -1/\boxed{1-1}}
{
    \node at (3.8,\y)[left]{$\ytext$};
}
...
用辗转相除法求最高公因式
\begin{enumerate}
    \item $f (x) =x^3-2x^2-2x-3,\quad g (x) =2x^3+x^2+x-1$
    \item $g(x)=\poly{1,3,2,3,1},\quad f(x)=\poly{2,5,-1,-1}$
    \item $f(x)=\poly{1,0,-2,9,-5,3},\quad g(x)=x^4-x^3+5x-3$
    \item 求$(6x^4-6x^2,\; 3x^2-3,\; 8x^8-8x^4,\; 7x^7-7x^3)$
    \item $f(x)=\poly{1,-2,-4,4,-3}$,\quad $g(x)=\poly{2,-5,-4,3}$,\quad $\varphi(x)=x^2-9$
\end{enumerate}
\end{ex}
...
如果一个整数$c$, 同时是数$a$和$b$的倍数，那么$c$就叫$a$、$b$的公倍数．而两个数的公倍数中，最小的一个正整数，叫做这两个数的\textbf{最小公倍数}．记作$[a,b]$．例如：4和6的公倍数有：$\pm 12,\pm 24,\pm 36,\cdots$, 但是，其中最小的一个正整数是12, 因此，$[4, 6]=12$．
...
例如：$x+1$与$x-1$的公倍式有：$x^2-1$, $2x^2-2$, $\frac{1}{3}x^2-\frac{1}{3}$, $x (x^2-1)$, $(x^2+1)(x^2-1),\cdots$．
...
\begin{enumerate}
    \begin{multicols}{2}
        \item $9 x^{2} y z,\quad -6 x y^{2} z,\quad 24 x y^{2}$
        \item $x^{2}-y^{2},\quad  x^{4}-y^{4}$
    \end{multicols}
    \item $-3(a-b)^{3},\quad -6(a-b)\left(a^{2}+a b+b^{2}\right),\quad -8(a-b)^{2}(a+b)$
    \item $m^{2}-4 n^{2},\quad  m^{2}+m n-2 n^{2},\quad m^{2}+3 m n+2 n^{2}$
\end{enumerate}
...
\begin{enumerate}
    ...
    \item $x^{2}-3 x y+2 y^{2},\quad  x^{2}+2 x y-8 y^{2}$
    \item  $m^{2}-m-6,\quad  m^{2}-2 m-3 $
    \item $y^{4}-9 y^{2},\quad  y^{3}-5 y^{2}+6 y$
    \item ${a}^{3}-3 a^{2}+2 a,\quad  a^{4}-a^{2},\quad  a^{2}+a-2$
    \item  $x^{2}-x-2,\quad  x^{2}+14 x-32,\quad  x^{2}-5 x+6$
\end{multicols}
    \item  $8 x^{3}-27,\quad 24 x-8 x^{2}-18,\quad 27-54 x+36 x^{2}-8 x^{3}$ 
    \item  $-6 x y\left(x^{2}-3 x y+2 y^{2}\right),\quad  4 x^{3} y^{2}\left(x^{2}-x y-2 y^{2}\right)$
    \item  $24 a^{4} b-24 a^{3} b^{2}+6 a^{2} b^{3}, \quad  36 a^{3} b^{2}-36 a^{2} b^{3}+9 a b^{4}$
\end{enumerate}

\item 求最高公因式与最低公倍式．
\begin{enumerate}
\item $\left(b^{2}-2 b\right)^{4},\quad   \frac{1}{8}\left(b^{2}-2 b\right)^{2},\quad  \left(b^{2}-2 b\right)^{3} ;$
\item $6\left(4 x^{2}-1\right),\quad   4-8 x ,\quad   12\left(4 x^{2}-4 x + 1\right)$
\item $12\left(x^{4}-y^{4}\right),\quad   10\left(x^{6}-y^{6}\right),\quad   8\left(x^{4} y+x y^{4}\right)$
\item $x^{2}-x, \quad  2 x^{2}-6 x,\quad   x^{3}-x$
\item $(x-1)(x-2),\quad   5 x^{4}-15 x^{3}+8 x^{2}+6 x-4$
\item $x^{3}-1,\quad   x^{3}-4 x^{2}-4 x-5$
\item $(x-1)^{2}(x-2)^{2},\quad (\poly{1,-3,2})(\poly{2,-5,5,-6})$
\item $(x^2-1)^2(x+1)^2,\quad (\poly{1,5,7,3})(\poly{1,-6,-7})$
\end{enumerate}

\item 用辗转相除法求最高公因式$(f(x),g(x))$．
\begin{enumerate}
    \item $f(x)=\poly{1,1,2,2},\quad g(x)=\poly{1,2,3,2}$
    \item $g(x)=\poly{2,-1,-1,1},\quad f(x)=\poly{1,-1,1,-2}$
    \item $f(x)=\poly{2,3,4,2,1},\quad g(x)=\poly{2,-1,2,0,1}$
    \item $g(x)=\poly{1,1,-1,1,2},\quad f(x)=\poly{2,5,-2,-7,2}$
    \item $f(x)=\poly{1,-2,1,-1,2,-1},\quad g(x)=\poly{5,-8,3,-2,2}$
\end{enumerate}

\item 求最低公倍式$[f(x),g(x)]$
\begin{enumerate}
    \item $g(x)=\poly{6,1,-44,21},\quad f(x)=\poly{3,-13,23,-21}$
    \item $f(x)=\poly{6,-3,7,1,-3},\quad g(x)=\poly{2,3,7,3,9}$
    \item $g(x)=\poly{1,0,0,0,1},\quad f(x)=\poly{1,0,0,-1}$
\end{enumerate}
...
\item  如果：已知$f(a_1),f(a_2),f(a_3),\cdots,f(a_{n+1})$
...
如果$f(x)$可以分解为$(mx+n)\cdot (px+q)\cdots (rx+s)$，那么，方程$f(x)=0$就一定有有理根$-\frac{n}{m},-\frac{q}{p},\cdots,-\frac{s}{r}$．

反过来，如果方程$f(x)=0$有有理根$-\frac{b}{a},-\frac{d}{c},\cdots,-\frac{f}{e}$．那么，多项式$f(x)$就一定含有因式：$(ax+b)(cx+d)\cdots(ex+f)$．
...
\[[f(x),g(x)]=\frac{f(x)\cdot g(x)}{(f(x), g(x))}\quad \text{(非零常数$k$不计)}\]
...
\item 求最高公因式及最低公倍式．
\begin{enumerate}
    \item $g(x)=\poly{1,1,-1,0,-2,-1},\quad f(x)=\poly{3,2,0,2,-3}$
    \item $g(x)=\poly{3,-5,4,-2,1},\quad f(x)=\poly{3,-2,1,-1}$
    \item $g(x)=\poly{1,2,3,2,0},\quad f(x)=\poly{1,1,2,2,0}$
\end{enumerate}
