...
\begin{solution}
\begin{enumerate}
    \item $\frac{18 a^{12} x^{3} y^{2}}{12 a^{3} b^{2} x^{5} z}=\frac{3 a^{9} y^{2}}{2 b^{2} x^{2} z}$
...
\begin{example}
    化简 $\frac{\poly{1,-2,0,1,-2}}{\poly{1,0,1,0,1}}$
\end{example}
...
\begin{example}
    化简 $\frac{\poly{1,-6,11,-6}}{\poly{1,-8,19,-12}}$  
\end{example}
...
\end{split}\]
$\therefore\quad \frac{\poly{1,-6,11,-6}}{\poly{1,-8,19,-12}}=\frac{x-2}{x-4}$
\end{solution}
...
\begin{solution}
    \[
        F(x)=\frac{-(x^3+1)}{2(x^2-x+1)}=-\frac{(x+1)(x^2-x+1)}{2(x^2-x+1)}=-\frac{x+1}{2}\quad        
F(5)=-\frac{5+1}{2}=-3
    \]
\end{solution}

\begin{example}
    化简$F(x)=\frac{2(x^2-1)(x-1)^2}{(1-x)^3(x+1)^2}$，问$x$取什么整数值时，能使$F(x)$的值是正整数．
\end{example}
...
\item 化简$F(x)=\frac{6x^2-12x+6}{x^3-3x^2+3x+1}$ 问$x$取何整数值，能使$F(x)$的值是正整数．
\end{enumerate}
\end{ex}
...
\begin{solution}
    \[
        \text{原式}=\frac{x+3y-(x+2y)+(2x-3y)}{x^2-y^2}=\frac{2x-2y}{x^2-y^2}=\frac{2(x-y)}{(x+y)(x-y)}=\frac{2}{x+y}
    \]
\end{solution}
...
\begin{analyze}
由余式定理得$x-1$是第一个分式的分子、分母的公因式，将此分式化简：
\[\frac{x^2-1}{x^4+x^2-2x}=\frac{(x+1)(x-1)}{(x-1)(x^3+x^2+2x)}=\frac{x+1}{x^3+x^2+2x}\]
由余式定理得$2x-1$是第二个分式的分子、分母的公因式，将此分式化简：
...
从以上两例可以看出：分式方程的两边乘以同一个含有未知数的整式，并进行化简，就得到一个新的整式方程．这个整式方程的根，可能是原分式方程的根，也可能不是原分式方程的根．而这里不适合原方程的根，就叫做原方程的\textbf{增根}，验根后应该舍去（例如，在例6.22中的$x=2$就是增根）．
...
解分式方程过程中，由于原方程两边乘以含有未知数的整式，化简而得到一个整式方程．这样就扩大了未知数的可取值范围，自然就有产生增根的可能．但是，增根并不可怕，只要通过检验，就可以鉴别出来把它舍去．所以，解分式方程是必须进行验根的．
...
\begin{solution}
方程两边乘以$x-1$，并化简得$x(x-1)=1$．

解整式方程：$x^2-x-1=0$，$\therefore\quad x_1=\frac{1}{2}+\frac{\sqrt{5}}{2},\qquad x_2=\frac{1}{2}-\frac{\sqrt{5}}{2}$．
...
但$\because\quad \frac{1}{2}-\frac{\sqrt{5}}{2}<0$，不合题意应舍去．$\therefore\quad $所求正实数是：$\frac{1}{2}+\frac{\sqrt{5}}{2}$．
\end{solution}

综合以上各例，可以概括出解分式方程的一般步骤是：
\begin{enumerate}
    \item 方程两边乘以分母的最低公倍式，并化简变
形为整式方程．
\item 解整式方程．
...
$\therefore\quad$原方程的解集是空集$\varnothing$．
\end{solution}
...
代入原方程就是 $y+\frac{1}{y}=2$，两边乘以$y$，并化简得
...
\begin{equation}
    (x-1)^2=x
\end{equation}
即：$x^2-3x+1=0$$\therefore\quad x_1=\frac{3}{2}+\frac{\sqrt{5}}{2},\qquad x_2=\frac{3}{2}-\frac{\sqrt{5}}{2}$．
...
$\because\quad a\ne b,\qquad \therefore\quad b-a\ne 0$\qquad$\therefore\quad x=ab$．

验根：把$x=ab$代入$abx$得$a^2b^2$．

$\because\quad a\ne 0,\; b\ne 0$ (如果$a=0$, $b=0$, 那么原方程无意义）．

$\therefore\quad a^2b^2\ne 0$\qquad$\therefore\quad $原方程的解集是$\{ab\}$．
\end{solution}
...
\begin{ex}
\begin{enumerate}
    \item 甲、乙两个车工，各车1500个螺丝．乙改进了操作方法，生产效率提高到甲的3倍，
    因此比甲少用20个工时完成任务．他们每小时各车多少个螺丝？
    \item 甲、乙两个车站相距96公里，快车和慢车同时从甲站开出，1小时后，快车在慢车
    前12公里，快车比慢车早40分钟到达乙站．快车和慢车的速度各是多少？
    \item 甲、乙、丙三人合做一件工作12天完成，已知甲1天完成的工作，乙须要2天，
    丙须要3天，问三人单独完成．这件工作，各需要多少天？
\end{enumerate}    
\end{ex}
...
\[1.\quad\sqrt{a^4},\qquad 2.\quad\sqrt{0.01a^2b^6},\qquad 3.\quad\sqrt{a^{2n}}\text{（}n\text{是自然数）} \]
\end{example}
...
\begin{example}
    把下列各根号外面的因式移到根号里面去．
\[1.\quad2a\sqrt{a},\qquad 2.\quad(m-n)\sqrt{m}\;\; (m-n\ge 0),\qquad3.\quad -x\sqrt{a}\;\;(x>0) \]
\end{example}
...
\begin{example}
    化去根号里面的分母
\[1.\quad\sqrt{\frac{3}{50}},\qquad 2.\quad\sqrt{\frac{c}{a^2b}}\quad (a>0,\; b>0),\qquad 3.\quad\sqrt{\frac{(a+b)^2}{a^2-b^2}}\quad (a>b)\]
\end{example}
...
\begin{example}
    把下列各根式化为最简根式，并指出哪些
是同类根式？
\[\sqrt{2},\qquad \sqrt{75},\qquad \sqrt{\frac{1}{50}},\qquad \sqrt{\frac{1}{27}},\qquad \sqrt{3}\frac{2}{3}\sqrt{8ab^3}\quad (b>0),\qquad 6b\sqrt{\frac{a}{2b}}\quad (b>0)\]
\end{example}
...
\begin{solution}
\begin{enumerate}
    \item $\text{原式}=\left(1+\frac{1}{2}+\frac{1}{3}+6\frac{1}{6}\right)\sqrt{5}
        =2\sqrt{5}$
    \item $\text{原式}=2\sqrt{3}-\frac{1}{2}\sqrt{3}-\frac{1}{3}\sqrt{3}+3\sqrt{2}
...
\end{enumerate}
    ...
    \begin{enumerate}
        ...
        \item $2 \sqrt{a^{3} b^{3} c},\frac{1}{2} \sqrt{4 a b^{5} c}$ 和 $3 \sqrt{\frac{c}{a b}}$
        \end{multicols}
\item $\sqrt{2 x},\sqrt{2 a^{2} x^{3}}\;\; (a>0)$ 和 $\sqrt{50 x y^{2}}\;\;(y>0)$.
    \end{enumerate}
    \item 合并下列各式中的同类根式：
\begin{enumerate}
...
\item 写出以下各根式的有理化因式：
\[3\sqrt{7},\quad 7-\sqrt{11},\quad 5\sqrt{3}+\sqrt{10},\quad \sqrt{2x^2-1},\quad x+x\sqrt{y},\quad a^2-b\sqrt{a+1},\qquad \sqrt{x+2}-\sqrt{x}\]
\end{enumerate}
\end{ex}
...
\begin{solution}
  \begin{enumerate}
        ...
        \item $\text{原式}=\frac{18\sqrt{2x^3}}{3\sqrt{3y}}=\sqrt{6\frac{2x^3}{3y}} =\frac{2x}{y}\sqrt{6x}  $
...
\begin{enumerate}
    \item \[
\text{原式 }=\frac{\left(\sqrt{a^{3} b}+\sqrt{ab^{3}}-ab\right)\cdot\sqrt{ab}}{\sqrt{ab}\cdot\sqrt{ab}}\\
=\frac{1}{ab}\left(a^{2} b+a b^{2}-a b \sqrt{ab}\right)\\
=a+b-\sqrt{ab}
    \]
    \item \[
...
&= \frac{2a+2\sqrt{a+b}\cdot \sqrt{a-b}}{2b}=\frac{a+\sqrt{a^2-b^2}}{b} 
    \end{split}\]
\end{enumerate}
\end{solution}
...
\begin{multicols}{2}
    \begin{enumerate}
        \item \quad $\frac{\sqrt{3}}{\sqrt{2}+\sqrt{3}+\sqrt{5}}$
        \item \quad $2\div \left(1-\sqrt{2}+\sqrt{3}\right)$
    \end{enumerate}
\end{multicols}
\end{example}
...
&=\frac{(1-\sqrt{2}-\sqrt{3})\cdot \sqrt{2}}{-\sqrt{2}\cdot \sqrt{2}}
...
$\therefore\quad x=2$是原方程的增根（舍去）．\qquad$\therefore\quad $原方程的解集是：$\{9\}$．
...
很明显，在例\ref{zengen}中，原方程的增根$x=2$就是方
程$\sqrt{x+7}=-(x-5)$的根．
...
解根式方程进行验根时，可以把有理方程的每一个根分别代入原根式方程的每一个根号内，
如果代入后，至少有一个根号内得负数，就说明这个根使原方程无意义，肯定是增根，
应该舍去（如例\ref{zengen2}）；如果
代入后，使原方程中的每一个根号内都为非负数，就
说明这个根能使原方程中的根式有意义，但还不能保
证能使原方程两边相等，仍然有可能是增根（如例\ref{zengen}），所以还应该继续代入原方程两边，进行检验．
...
\begin{enumerate}
    \begin{multicols}{2}
     \item $\sqrt{ab}+2 \sqrt{\frac{b}{a}}+\sqrt{\frac{1}{a b}}-6 \sqrt{\frac{a}{b}}$
...
\item $\left(\frac{a}{b} \sqrt{\frac{n}{m}}-\frac{a b}{n} \sqrt{m n}+\frac{a^{2}}{b^{2}} \sqrt{\frac{m}{n}}\right) a^{2} b^{2} \sqrt{\frac{n}{m}}$
...
分式方程的特点是：分母中含有未知数．因而，要利用分式的基本性质或等式的基本性质，两边乘以同一个整式（一般是取各分母的最低公倍式），化简后
转化为一个整式方程．
...
\item \begin{enumerate}
    ...
    \item 已知$x=\frac{2ab}{b^2+1}$，其中$a,b$都是正数，求 $\frac{\sqrt{a+x}+\sqrt{a-x}}{\sqrt{a+x}-\sqrt{a-x}}$的值．
\end{enumerate}
...
\item 设 $x=\sqrt{3+\sqrt{5}},\quad  y=\sqrt{3-\sqrt{5}}$,求 $\frac{x+y}{x-y}-\frac{x-y}{x+y}$-的值.
...
\begin{enumerate}
    ...
    \item 若 $\frac{y}{x}+\frac{x}{z}=a,\quad  \frac{z}{y}+\frac{y}{x}=b,\quad  \frac{x}{z}+\frac{z}{y}=c$求证 $(a+b-c)(a-b+c)(-a+b+c)=8$．
\end{enumerate}
