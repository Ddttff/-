...
在这两串数$1, 2,\cdots,100$和$1, 3, 5,\cdots,99$中都有一个特点：从第二个数起；每个数与它前边的一个数
的差都相等．像第一个数串中，这个差为1；第二个数串中，这个差为2．

像这样有次序地排好的一串数，其中任一数与它前一个数的差都相等．我们就把这样一串数叫做\textbf{等差数列}．这个“相等的差”，叫做这个数列的\textbf{公差}．数列中的每一个数，叫做这个数列的一项．排头的一个数，叫首项，排尾的一个数叫末项．
...
\begin{solution}
\begin{enumerate}
    \item 首项为2, 公差为5的等差数列各项为：$2, 7, 12, \cdots, 2+5 (n-1) ,\cdots$
    \item 首项为2, 公差为$-1$的等差数列的各项为：
    $2, 1, 0, -1, -2, \cdots, 2+ (n-1)\cdot(-1) ,\cdots$
\end{enumerate}
\end{solution}
...
一般地，对于等差数列$a_1,a_1+d,a_1+2d,\cdots,a_1+(n-1)d,\cdots$的前$n$项求和公式，可作如下推导：
...
可得：$1000=\frac{25(6+24d)}{2}$．\qquad$\therefore\quad d=\frac{74}{24}=\frac{37}{12}$
\end{solution}
...
首项是2, 公比是$-1$的等比数列是：
\[2, -2, 2, \cdots, (-1)^{n-1}\cdot 2,\cdots\]
\end{solution}
...
\begin{enumerate}
    ...
    \begin{enumerate}
        \begin{multicols}{3}
        \item $4, 12, 36, 108,\cdots$
        \item $5, -10, 20,-40,\cdots$
        \item $2, 1, \frac{1}{2}, \frac{1}{4},\cdots$
        \end{multicols}
...
一般地，对于等比数列$a_1,a_1q,a_1q^2,\cdots,a_1q^{n-1},\cdots$的前$n$项求和，我们可作如下推导：
...
而当$q=1$时，这个等比数列为：$a_1,a_1,a_1,\cdots,a_1,\cdots$
...
\[
    ...
    =\frac{1\left(1-\frac{1}{2^{10}}\right)}{1-\frac{1}{2}}\\
...
\begin{enumerate}
    \begin{multicols}{2}
    \item \quad$3+6+12+24+\cdots+3072$
    \item \quad$1-2+4-8+\cdots-512+1024$
    \item \quad$1+x+x^2+\cdots+x^{n-1}\qquad (x\ne 1)$
    \end{multicols}
\end{enumerate}
\end{ex}
...
\begin{enumerate}
    \begin{multicols}{2}
    \item $5,\; 11,\; 17,\; 23, \cdots $
    \item $2,\; 5,\; 8,\; 11, \cdots $
    \item $-13,\; -11,\; -9,\; -7, \cdots $
    \item $-\frac{3}{2},\; -\frac{2}{3},\;  \frac{1}{6}, \; 1, \cdots $
    \item $(4 a-3 b),\; (2 a+b),\;  5 b,\; (9 b-2 a), \cdots$
    \item $(a+3 m),\; (a+m),\; (a-m),\; (a-3 m) \cdots $
    \end{multicols}
...
\begin{enumerate}
    \begin{multicols}{2}
     \item $2,\; 7,\; 12, \cdots $ 的前 10 项；
     \item $15,\; 8,\;1, \cdots $ 的前15项；
     \item $\frac{1}{6},\; \frac{4}{3},\; \frac{5}{2}, \cdots $ 的前 14 项；
     \item $(x-y),\; x,\;(x+y), \cdots$ 的前30项；
     \item $(x-y),\; y,\;(-x+3 y), \cdots$ 的前 25 项．
    \end{multicols}
\end{enumerate}
...
\begin{enumerate}
    ...
    \item $4,\; 4,\; 4,\; 4,\cdots$
    \item $4,\; 12,\; 36,\; 108, \cdots$
    \item $-24,\; 12,\; -6,\; 3,\; -\frac{3}{2},\cdots$
    \item $5x^3,\; 10x^2,\; 20x,\; 40, \cdots \qquad  (x\ne 1)$
    \item $\frac{x}{y},\; x,\; xy,\; xy^2,\cdots\qquad (y\ne 1)$
    \end{multicols}
\end{enumerate}
...
\begin{enumerate}
    \begin{multicols}{2}
    \item $1,\; 3,\; 9,\; 27,\cdots$的前10项；
    \item $2,\; -2,\; 2,\;-2,\cdots$的前101项；
    \item $625,\; 125,\; 25,\; 5,\; 1,\cdots$的前10项；
    \item $56,\;-28,\; 14,\;-7,\cdots$的前10项．
    \end{multicols}
\end{enumerate}
...
\item *如果有一数列是：$100,\; 95,\; 90,\; 85,\cdots$
...
像以上例题的解题方法，叫做待定系数法（或叫未定系数法），这个方法的特点是引进待定系数（未知的），列出一个含有待定系数的恒等式，然后根据多项式恒等的性质，比较等式两边的同类项系数，得出一个方程组．解这个方程组，求出待定系数，或消去待定系数而找出原来已知系数之间所存在的关系，使问题得以解决．
...
那么，$a_n=b_n,\; a_{n-1}=b_{n-1},\cdots, a_1=b_1,\; a_0=b_0$
...
所以，$a_n=b_n,\; a_{n-1}=b_{n-1},\cdots, a_1=b_1,\; a_0=b_0$
\end{proof}
...
\begin{itemize}
    \item 令$x=\frac{1}{2}$，代入恒等式(7.12)得：$\frac{35}{4}=a\cdot \frac{35}{4}$\qquad$\therefore\quad a=1$
    \item 令$x=-3$，代入恒等式(7.12)得：$-168=(-42)b$\qquad  $\therefore\quad b=4$
    \item 令$x=3$，代入恒等式(7.12)得：$-30=45c$\qquad$\therefore\quad c=-\frac{2}{3}$
\end{itemize}
...
不妨：
\begin{itemize}
    \item 令$x=1$, 得$6=3c$, \qquad$\therefore\quad c=2$
    \item 再令$x=-2$, 得$9=9a$, \qquad$\therefore\quad a=1$
\item 再令$x=0$, 得$-1=a-2b+2c$, \qquad$\therefore\quad b=3$
\end{itemize}
因此：
\[\begin{split}
    \frac{4x^2+3x-1}{(x-1)(x^2+x-2)}&=\frac{4x^2+3x-1}{(x-1)^2(x+2)}\\
...
其中，$m_1,n_1,m_2,n_2,\cdots,m_n,n_n$
都是常数（$a\ne 0$）”．
但运算较繁，这里就略去了．
...
\begin{figure}[htp]
    \centering
    \includegraphics[scale=.7]{image/1.png}
\end{figure}
...
\begin{itemize}
    ...
    \item 求数列$1^b,\; 2^b,\; 3^b, \cdots,n^b,\cdots$ 前$n$项和（$b$是大于1的一个自然数）．
\end{itemize}
...
\begin{enumerate}
    ...
    \item $-0.1,\;0.1,\;0.3,\;0.5 \cdots$；
    \item $9,\;3,\;1,\; \frac{1}{3},\; \frac{1}{9} \cdots$．
    \end{multicols}
\end{enumerate}

\item 试求出下列数列的第 $n$ 项, 并导出它的前 $n$ 项求和公式：
\begin{enumerate}
    \item $1 \frac{1}{2},\; 2 \frac{1}{4}, \;3 \frac{1}{8} ,\; 4 \frac{1}{16}, \cdots$；
    \item $ 1.3,1.03,1.003,1.0003, \cdots$；
    \item *$\frac{1}{1 \times 2},\; \frac{1}{2 \times 3},\; \frac{1}{3 \times 4},\; \frac{1}{4 \times 5},\cdots$．
\end{enumerate}

\item 如果有三个数成等差数列, 又成等比数列, 那么, 你能
    说明这三个数一定相等吗?
    \item 首项为100,公差为$-10$的等差数列中，前多少项的和最大？求出这个和．
\item  *如果 $a, b, c$ 分别是一等比数列的第 $p, q, r$ 项, 试说明：
